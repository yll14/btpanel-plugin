 ...
 * @returns {object} 返回 API 响应数据
 */
async function btPost(path, Other = {}) {
  let config,API_SK,sign,bt_url;
  config = GetConfig(`config`, `Config`);
  API_SK = config.API_SK;
  sign = getBTSign(API_SK);
  ...
  try {
    const response = await axios.post(bt_url + path, 
      new URLSearchParams({
...
export { _PATH, PluginName_zh, PluginName_en, PluginPath, GetConfig, image, getBTSign, btPost };